...
body_.Connect = function(i) {
    if (!i || typeof i != "object" || !i.url) throw TypeError(`${"[".bold}${"error".red.bold}${"]".bold} ${"faltando 'url' em .create({ url:... })".bold.red}`);
    ...
    this.Get = async function(key_, id) {
        if (!key_) throw TypeError(`${"[".bold}${"error".red.bold}${"]".bold} ${"faltando 'key' em get()".bold.red}`);
        if (!id) throw TypeError(`${"[".bold}${"error".red.bold}${"]".bold} ${"faltando 'id' em get()".bold.red}`);
        let user = await data.findOne({ key: `${key_}_${id}` });
        let key__ = _baseValues_[key_];
        if (!key__) throw TypeError(`${"[".bold}${"error".red.bold}${"]".bold} ${"você não definiu ".bold.red + key_.bold.yellow + " em .Variables({...})".bold.red}`);
        if (!user) {
            user = key__;
    ...
    this.Set = async function(key_, user_, value) {
        if (!key_) throw TypeError(`${"[".bold}${"error".red.bold}${"]".bold} ${"faltando 'key' em .set({...})".bold.red}`);
        if (!_baseValues_[key_]) throw TypeError(`${"[".bold}${"error".red.bold}${"]".bold} ${"você não definiu ".bold.red + key_.bold.yellow + " em .Variables({...})".bold.red}`);
        if (!user_) throw TypeError(`${"[".bold}${"error".red.bold}${"]".bold} ${"faltando 'id' em .set({...})".bold.red}`);
